/* v52 — vertical grouped bar drill-down for answered questions in comparison */
(()=>{
  const MULTI_KEY='bamco-multi-vehicle-comparisons';
  const COLORS=['#195b8e','#12877e','#7454b8','#d58a24','#b54d66','#2c7d9a'];

  const fa=()=>typeof locale==='undefined'||locale==='fa';
  const escHtml=value=>typeof esc==='function'?esc(String(value??'')):String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const shown=value=>typeof localNumber==='function'?localNumber(value):String(value);
  const criteria=()=>window.ASSESSMENT_CRITERIA||[];
  const criterionTitle=main=>{
    try{return typeof criterionLabel==='function'?criterionLabel(main):(fa()?(main.titleFa||main.id):(main.titleEn||main.titleFa||main.id))}catch(_){return fa()?(main.titleFa||main.id):(main.titleEn||main.titleFa||main.id)}
  };
  const questionTitle=item=>fa()?(item.titleFa||item.textFa||item.id):(item.titleEn||item.titleFa||item.textFa||item.id);
  ...
  function storedCases(){
    try{
      const parsed=JSON.parse(sessionStorage.getItem(MULTI_KEY)||'[]');
  ...
  function answeredQuestions(main,cases){
    ...
    (main.subgroups||[]).forEach(sub=>(sub.items||[]).forEach(item=>{
      ...
      const values=cases.map(record=>{
        const n=Number(record?.state?.scores?.[key]);
        return Number.isFinite(n)&&n>=1&&n<=10?n:NaN;
      });
  ...
  function backToComparison(){
    try{
      if(typeof managerRefresh==='function'){managerRefresh();return}
      if(typeof window.managerRefresh==='function'){window.managerRefresh();return}
    }catch(_){ }
    location.reload();
  }
  ...
  function renderVerticalBars(criterionIndex){
    injectStyles();
    const main=criteria()[criterionIndex];
    const body=document.querySelector('#managerComparisonCard #vehicleComparisonBody');
    if(!main||!body)return;

    const cases=allCases();
    const names=cases.map(vehicleName);
    const questions=answeredQuestions(main,cases);
  ...
  function criterionIndexFromEventTarget(target){
    const label=target?.closest?.('[data-radar-detail-index]');
    if(label){const n=Number(label.dataset.radarDetailIndex);return Number.isInteger(n)?n:null}
    const axis=target?.closest?.('.radarDrillTarget');
    if(axis){const n=Number(axis.dataset.index);return Number.isInteger(n)?n:null}
    return null;
  }

  document.addEventListener('click',event=>{
    if(!event.target?.closest?.('#managerComparisonCard'))return;
    const index=criterionIndexFromEventTarget(event.target);
    if(index===null)return;
    event.preventDefault();
    event.stopImmediatePropagation();
    renderVerticalBars(index);
  },true);
