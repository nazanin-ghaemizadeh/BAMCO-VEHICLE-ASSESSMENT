(()=>{
  ...
  if(page==='roles'){
    const user=readUser();
    if(!user){window.location.replace(base('index.html?v=80'));return}
    ...
    document.querySelector('#signOutButton').addEventListener('click',()=>{
      ...
      window.location.replace(base('index.html?v=80'));
    });
    return;
  }

  if(readUser()){window.location.replace(base('roles.html?v=80'));return}
  ...
  form.addEventListener('submit',event=>{
    ...
    if(users.has(user)&&password.value===PASSWORD){
      ...
      window.location.replace(base('roles.html?v=80'));return;
    }
