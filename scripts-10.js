(()=>{
  ...
  function ensureHeaderNavigation(){
    const actions=document.querySelector('.headerActions .actionCard');
    if(!actions)return null;
    let row=actions.querySelector('.v55ComparisonNavRow');
    if(!row){
  ...
  function nav(){
    ...
    const inComparison=document.documentElement.classList.contains('managerComparisonMode') && !!document.querySelector('#managerComparisonCard:not([hidden])');
    const barBack=document.querySelector('#managerComparisonCard #v52BackButton');
    const detailBack=document.querySelector('#managerComparisonCard #backToComparisonOverview');
  ...
  function radarMode(){
    try{return sessionStorage.getItem(RADAR_KEY)==='overlap'?'overlap':'separate'}catch(_){return'separate'}
  }
  function saveRadarMode(mode){
    try{sessionStorage.setItem(RADAR_KEY,mode==='overlap'?'overlap':'separate')}catch(_){}
  }
  ...
  function buildOverlayRadar(grid,sourcePanes){
    let overlay=grid.querySelector(':scope > .v55OverlayPane');
    if(overlay)return overlay;
    const sourceWithSvg=sourcePanes.find(p=>p.querySelector('svg'));
    const sourceSvg=sourceWithSvg?.querySelector('svg');
    ...
    const insertBefore=svg.querySelector('line')||svg.querySelector('text')||null;
    let seriesCount=0;
    sourcePanes.forEach((pane,index)=>{
      const polygon=pane.querySelector('svg polygon.radarData');
      if(!polygon)return;
      const series=polygon.cloneNode(true);
      const color=SERIES_COLORS[index%SERIES_COLORS.length];
      series.classList.add('v55RadarSeries');
      series.removeAttribute('style');
      series.style.fill=color;
      series.style.fillOpacity='.14';
      series.style.stroke=color;
      series.style.strokeWidth='3';
      series.style.pointerEvents='none';
      if(insertBefore)svg.insertBefore(series,insertBefore);else svg.appendChild(series);
      seriesCount++;
    });
    if(!seriesCount)return null;

    overlay=document.createElement('article');
    overlay.className='comparisonRadarPane v55OverlayPane';
    const titleEl=document.createElement('strong');
    titleEl.textContent=document.body.classList.contains('english')?'Overlaid Comparison':'مقایسه هم‌پوشانی';
    ...
    const chart=document.createElement('div');chart.className='comparisonManagerRadar radarChart v55OverlayChart';chart.appendChild(svg);
    overlay.append(titleEl,legend,chart);grid.appendChild(overlay);return overlay;
  }
  function ensureRadarModeControl(grid){
    const card=grid.closest('.comparisonChartCard');
    const header=card?.querySelector('.comparisonChartHeader');
    if(!header)return null;
    let control=header.querySelector('.v55RadarModeControl');
    if(!control){
      control=document.createElement('div');control.className='v55RadarModeControl';
      control.innerHTML='<button type="button" data-radar-mode="separate"></button><button type="button" data-radar-mode="overlap"></button>';
      control.addEventListener('click',event=>{
        const button=event.target.closest('[data-radar-mode]');if(!button)return;
        saveRadarMode(button.dataset.radarMode);applyRadarView();
      });
      header.appendChild(control);
    }
    const english=document.body.classList.contains('english');
    const separate=control.querySelector('[data-radar-mode="separate"]');
    const overlap=control.querySelector('[data-radar-mode="overlap"]');
    if(separate)setText(separate,english?'Separate':'جدا از هم');
    if(overlap)setText(overlap,english?'Overlaid':'هم‌پوشانی');
    return control;
  }
  function applyRadarView(){
    const grid=document.querySelector('#managerComparisonCard .multiRadarGrid');
    if(!grid)return;
    const panes=[...grid.children].filter(el=>el.classList?.contains('comparisonRadarPane')&&!el.classList.contains('v55OverlayPane'));
    if(panes.length<2)return;
    const control=ensureRadarModeControl(grid);if(!control)return;
    ...
    if(mode==='overlap'){
      const overlay=buildOverlayRadar(grid,panes);
      if(overlay){overlay.hidden=false;grid.classList.add('v55OverlapMode')}
      else grid.classList.remove('v55OverlapMode');
    }else{
      grid.classList.remove('v55OverlapMode');
      const overlay=grid.querySelector(':scope > .v55OverlayPane');if(overlay)overlay.hidden=true;
    }
  }

  function css(){
    let s=document.querySelector('#bamco-v53-styles');
    if(!s){s=document.createElement('style');s.id='bamco-v53-styles';document.head.appendChild(s)}
    const text=`html{width:100%;max-width:100%;overflow-x:hidden;-webkit-text-size-adjust:100%;text-size-adjust:100%;-webkit-tap-highlight-color:transparent}body{width:100%;max-width:100%;overflow-x:hidden;min-width:0}body:not(.english),body:not(.english) button,body:not(.english) input,body:not(.english) select,body:not(.english) textarea,body:not(.english) label,body:not(.english) h1,body:not(.english) h2,body:not(.english) h3,body:not(.english) h4,body:not(.english) p,body:not(.english) span,body:not(.english) strong,body:not(.english) small,body:not(.english) table,body:not(.english) th,body:not(.english) td{font-family:"B Nazanin","B Zar","B Yas","Nazanin","B Nazanin Regular",Tahoma,Arial,sans-serif!important}img,svg,canvas,video{max-width:100%;height:auto}button,input,select,textarea{max-width:100%;min-width:0}main,.card,.mainCriterion,.entryCard,.comparisonWorkspace,.comparisonWorkspaceHeader,.comparisonDetailHeader{min-width:0;max-width:100%}#managerComparisonCard .v52ChartScroller{width:100%!important;display:block!important}#managerComparisonCard .v52Chart{width:max-content!important;min-width:max-content!important;margin-left:auto!important;margin-right:auto!important}#managerComparisonCard .v52DetailTitle h3{text-align:center!important}#managerComparisonCard .comparisonWorkspaceHeader>#closeComparisonMode,#managerComparisonCard #v52BackButton,#managerComparisonCard #backToComparisonOverview{display:none!important}.headerActions .actionCard .v55ComparisonNavRow{flex:0 0 100%!important;width:100%!important;display:flex;align-items:center;gap:8px;justify-content:flex-start;order:999;padding-top:2px}.headerActions .actionCard .v55ComparisonNavRow[hidden]{display:none!important}.v55TopNavButton{margin:0!important;min-height:42px!important;background:#f4f8fb!important;border:1px solid #bfd0dd!important;color:#173f63!important}.v55RadarModeControl{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-inline-start:auto}.v55RadarModeControl button{min-height:36px;padding:6px 12px;border:1px solid #c9dbe5;background:#f6f9fb;color:#36566e;border-radius:10px;font-size:12px;font-weight:850}.v55RadarModeControl button.active{background:linear-gradient(135deg,#164e79,#11857e);border-color:transparent;color:#fff}.multiRadarGrid.v55OverlapMode{grid-template-columns:1fr!important}.multiRadarGrid.v55OverlapMode>.comparisonRadarPane:not(.v55OverlayPane){display:none!important}.v55OverlayPane{width:100%!important;max-width:1000px!important;margin-inline:auto!important}.v55RadarLegend{display:flex;align-items:center;justify-content:center;gap:10px 18px;flex-wrap:wrap;margin:8px 0 4px}.v55RadarLegendItem{display:inline-flex;align-items:center;gap:7px;color:#405d74;font-size:12px;font-weight:800}.v55RadarLegendSwatch{width:12px;height:12px;border-radius:50%;flex:0 0 auto}.v55RadarSeries{vector-effect:non-scaling-stroke}.scoreChart{border-bottom:0!important;box-shadow:none!important;scrollbar-width:none!important;-ms-overflow-style:none!important}.scoreChart::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}.scoreChart::before,.scoreChart::after{display:none!important;content:none!important}#managerComparisonCard .v52YAxis span:first-child{top:0!important;bottom:auto!important;transform:none!important}#managerComparisonCard .v52Bar[style*="--h:100%"] .v52BarValue{top:8px!important;color:#fff!important;text-shadow:0 1px 2px #0008!important}@supports(padding:max(0px)){body{padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right)}.topbar{padding-left:max(14px,env(safe-area-inset-left));padding-right:max(14px,env(safe-area-inset-right))}main{padding-left:max(10px,env(safe-area-inset-left));padding-right:max(10px,env(safe-area-inset-right));padding-bottom:max(30px,env(safe-area-inset-bottom))}}@media(max-width:900px){body{font-size:14px!important}.topbar{width:100%!important;max-width:100%!important}.brand{min-width:0!important}.brand>div{min-width:0!important}.brand strong,.brand small{white-space:normal!important}.headerActions{flex-wrap:wrap!important;min-width:0!important}main{width:100%!important;max-width:100%!important;padding:14px 10px 30px!important}.card{padding:18px 16px!important;width:100%!important;max-width:100%!important}.sectionTitle{align-items:flex-start!important;flex-wrap:wrap!important}.sectionTitle h2{font-size:20px!important;line-height:1.45!important}.sectionTitle p{font-size:13px!important;line-height:1.75!important}.managerWeightPanel{min-height:250px!important}.managerWeightPanel .sectionTitle{gap:10px!important;align-items:flex-start!important}.managerWeightPanel .sectionTitle p{white-space:normal!important;overflow:visible!important}.entryTabPanel[hidden]{display:none!important}.entryTabButton{font-size:14px!important}.fieldGrid,.dashboard,.summaryGrid,.strengthWeaknessGrid{grid-template-columns:1fr!important}.actionCard{display:grid!important;grid-template-columns:1fr!important;width:100%!important}.actionCard>*{width:100%!important;min-width:0!important}.headerActions .actionCard .v55ComparisonNavRow{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important}.headerActions .actionCard .v55ComparisonNavRow[hidden]{display:none!important}.tableWrap,.managementStyleComparisonTable,.multiVehicleComparisonTable{max-width:100%!important;overflow-x:auto!important;-webkit-overflow-scrolling:touch!important}.criteriaNav{position:static!important}.criteriaNav nav{display:block!important}.assessmentLayout{grid-template-columns:1fr!important}.item{grid-template-columns:1fr!important}.comparisonCountPrompt,.comparisonCountToolbar,.comparisonUploadSlot{grid-template-columns:1fr!important}.multiComparisonUploadGrid,.multiRadarGrid,.comparisonRadarPair{grid-template-columns:1fr!important}.comparisonRadarPane{min-height:0!important;overflow:hidden!important}.comparisonManagerRadar.radarChart svg{width:100%!important;max-width:100%!important;height:auto!important;min-height:320px!important}.v55RadarModeControl{width:100%!important;margin:8px 0 0!important;display:grid!important;grid-template-columns:1fr 1fr!important}.v55RadarModeControl button{width:100%!important}#managerComparisonCard .comparisonWorkspaceHeader,#managerComparisonCard .comparisonDetailHeader{display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:12px!important;text-align:right!important}#managerComparisonCard .comparisonWorkspaceTitle,#managerComparisonCard .comparisonWorkspaceHeader>div,#managerComparisonCard .comparisonDetailHeader>div{width:100%!important;min-width:0!important;order:1!important;text-align:right!important}#managerComparisonCard .comparisonWorkspaceHeader h2,#managerComparisonCard .comparisonWorkspaceHeader h3,#managerComparisonCard .comparisonDetailHeader h3{width:100%!important;margin:0 0 6px!important;line-height:1.55!important;text-align:right!important}#managerComparisonCard .comparisonWorkspaceHeader p,#managerComparisonCard .comparisonDetailHeader p{width:100%!important;margin:0!important;text-align:right!important}#managerComparisonCard .comparisonWorkspaceHeader>span,#managerComparisonCard .comparisonDetailHeader>span{display:none!important}input,select,textarea{font-size:16px!important}input[type=date],input[data-meta*=date i],input[id*=date i]{font-size:16px!important;scroll-margin-block:30vh}input.mobileCalendarFocus{position:relative!important;z-index:5!important;transform:scale(1.03);transform-origin:center;box-shadow:0 0 0 4px #1b7fa52b,0 8px 22px #173f6330!important}}@media(max-width:480px){body{font-size:13.5px!important}main{padding-inline:8px!important}.card{padding:15px 13px!important;border-radius:14px!important}.sectionTitle h2{font-size:18px!important}.sectionTitle p{font-size:12.5px!important}.brand strong{font-size:20px!important}.brand small{font-size:12px!important}.entryTabButton,button,.button{white-space:normal!important;line-height:1.55!important}.scoreChart{gap:4px!important}.barItem{min-width:56px!important}.barTrack{width:32px!important}}`;
  ...
  let q=false;
  function refresh(){css();labels();nav();applyRadarView()}
  function schedule(){if(q)return;q=true;requestAnimationFrame(()=>{q=false;refresh()})}
  new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true,characterData:true});
  document.addEventListener('click',top,true);
  document.addEventListener('focusin',centerDate,true);
  document.addEventListener('focusout',blurDate,true);
